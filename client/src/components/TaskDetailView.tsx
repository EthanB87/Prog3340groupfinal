...
const ensureDefaults = (t: Task): Task => ({
  ...t,
...
export function TaskDetailView({
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        setIsLoading(true);
        setError(null);
        const [rawTask, userList] = await Promise.all([
          fetchTaskById(apiBaseUrl, taskId),
          fetchUsers(apiBaseUrl).catch(() => [] as UserSummary[]),
        ]);

        const payload = rawTask as TaskResponse;
  ...
  const getPriorityIcon = () => {
    switch (viewTask?.priority || "medium") {
      case "highest":
  ...
  const getPriorityLabel = () => {
    if (!viewTask) return "";
    const priority = viewTask.priority || "medium";
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <div className="text-gray-900">
                    {formatDate(viewTask.createdAt)}
                  </div>
                </div>
                <div>
                  <div className="text-gray-600 mb-1">Updated</div>
                  <div className="text-gray-900">
                    {formatDate(viewTask.updatedAt)}
                  </div>
                </div>
                <div>
